...
const Masonry: React.FC<MasonryProps> = ({
  ...
  const handleMouseMove = useCallback((id: string, element: HTMLElement, e: React.MouseEvent) => {
  ...
  }, [grid, enableTilt, enableMagnetism]);

  const handleMouseLeaveCard = useCallback((id: string) => {
    if (enableTilt) {
      gsap.to(`[data-key="${id}"]`, {
        rotateX: 0,
        rotateY: 0,
  ...
  const handleClick = (id: string, element: HTMLElement, e: React.MouseEvent) => {
    ...
    <div ref={containerRef} className="relative w-full" style={{ minHeight: '600px' }}>
      {grid.map(item => (
        <div
          ...
          onClick={(e) => {
            handleClick(item.id, e.currentTarget, e);
            if (onItemClick) {
              onItemClick(item.id);
            } else {
              // Handle URL - if it's a hash, don't open in new tab
